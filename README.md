...
**2026-09 v3.0.2**：基于已验证可用的 v3.0.0 CUPS Raster 路径，合并逆向结论：`SDRST` 只重置条带本地参考状态、在每个 SDRST 条带边界完整重置 arithmetic state；固定 `L0=128`、64 行条带对齐，并为输入页加入 128 行顶部空白条带。**v3.0.1 完全不可用，不作为基础版本。**
...
| **半色调** | 8×8 Bayer 有序抖动 |
| **固件 workaround** | 顶部 128 行空白 padding + SDRST 仅重置条带本地参考状态 |
...
1. **页面顶部必须 ≥1 个空白 stripe**（128 行 @600dpi）：否则打印机"轰鸣不出纸"。过滤器固定添加 128 行空白。
2. **SDRST 条带边界**：输出 `SDRST`，保留 arithmetic probability state，只重置条带本地参考状态。
3. **条带参数**：`L0=128`，保持 64 行条带对齐。
...
| 现象 | 可能原因 | 解决方法 |
|------|---------|---------|
| 轰鸣不出纸 | 页面顶部缺少 128 行空白条带 | 过滤器固定添加 128 行顶部空白 |
| 页面下部大块黑域/乱码 | SDRST 重置了错误的状态 | 保留 arithmetic probability state，仅重置条带本地参考状态 |
...
- **Firmware workarounds** (discovered via physical testing):
  - 128 blank rows at page top (fixed)
  - SDRST at each 128-row stripe boundary; arithmetic state is reset at each stripe boundary (required by M100D hardware)
